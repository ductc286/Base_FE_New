...
import MenuSearchListModal from "@src/Cores/Components/ShareComponents/SearchLists/MenuSearchListModal";
...
export default function AuthorizationMenuPage() {
  ...
  const tableRef = useRef<TableRefTcd<AuthorizationMenuViewModel>>(null);
  ...
  const columnConfig: TableColumnTcdModel<AuthorizationMenuViewModel>[] =
    useMemo(
      () => [
        {
        ...
        },
        {
          ...
          sortable: true,
          width: 100,
          render: (param: ColumnRenderParamTcd<AuthorizationMenuViewModel>) =>
  ...
  const [listData, setListData] = useState<AuthorizationMenuViewModel[]>();
  ...
  //#region functions
  function initData(filterParam?: CoreMenuFilter, coreRoleId?: string) {
    ...
    AppAuthenticationService.GetAuthorizeAction(
      FeatureCodeCommonConst.authorizationMenu
    ).then((res) => {
      if (res.isSuccess) {
  ...
  function handleGetList(
    ...
    CoreAuthorizationReferenceService.GetListMenuByRole(
      coreRoleIdParam,
      filterRequest,
      ...
      .then((res) => {
        if (res.isSuccess) {
          const dataConvert = res.data?.map((x) => {
            return {
              ...x,
              [CommonConst.keyDisableItemInTable]:
                x.authorizeType === MenuAuthorizeTypeEnum.anonymous ||
                x.authorizeType === MenuAuthorizeTypeEnum.logged,
            };
          });
  ...
  function deleteClick() {
    ...
    useConfirmModalState.confirm({
      ...
      onOk: () => {
        ...
        CoreAuthorizationReferenceService.DeleteListMenu(
          selectedRoleId,
          requestData
        )
          .then((res) => {
  ...
  function handleAdd(selectedIdParams?: string[]) {
    ...
    CoreAuthorizationReferenceService.AddListMenu(selectedRoleId, requestData)
      .then((res) => {
        if (res.isSuccess) {
          useToastState.showToast(NotifyTypeEnum.success, res.message);
    ...
    <>
      <ContainerTcd>
        ...
        >
          ...
          <CardTcd
            title={"Danh sách menu được phép truy cập"}
          ...
          >
            ...
            <TableTcd<AuthorizationMenuViewModel>
