...
import CorePageService from "@src/Cores/Services/Admin/Post/CorePageService";
import {
  CorePostEntity,
...
export default function PageFormCpn(props: Readonly<FormPropsModel<unknown>>) {
  //#region constants
  ...
  function getData(formModeParam?: FormModeEnum, recordId?: string) {
    ...
    CorePageService.Get(recordId ?? currentRecordId)
      .then((res) => {
        if (res.isSuccess) {
          loadData(
  ...
  function add(formDataParam: CorePostEntity) {
    ...
    CorePageService.Add(formDataParam)
      .then((res) => {
        if (res.isSuccess) {
  ...
  function edit(formDataParam: CorePostEntity) {
    ...
    CorePageService.Update(formDataParam)
      .then((res) => {
        if (res.isSuccess) {
          useToastState.showToast(NotifyTypeEnum.success, res.message);
          getData();
          ...
          {authorizeAction.requestApproval && (
            <ButtonTcd
              ...
              onClick={() => {
                ...
                CorePageService.RequestApprove({
              ...
              onClick={() => {
                useConfirmModalState.confirm({
                  ...
                  onOk: (note?: string) => {
                    ...
                    CorePageService.CancelRequestApproval({
                      recordId: formData.corePostId,
                      note: note,
                    })
                      .then((res) => {
              ...
              onClick={() => {
                useConfirmModalState.confirm({
                  ...
                  onOk: () => {
                    ...
                    CorePageService.Delete(formData.corePostId)
                      .then((res) => {
                        if (res.isSuccess) {
                          useToastState.showToast(
                            NotifyTypeEnum.success,
          ...
          <UpLoadTcd
            ...
            onChange={(dataPram: InputChangeModel) => {
              dataPram.value.length > 0 &&
                CorePageService.UploadImage(dataPram.value[0]).then((res) => {
                  if (res.isSuccess) {
                    useToastState.showToast(
                      NotifyTypeEnum.success,
