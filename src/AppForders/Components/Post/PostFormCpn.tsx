...
export default function PostFormCpn(props: Readonly<FormPropsModel<unknown>>) {
  //#region constants
  ...
  function getData(formModeParam?: FormModeEnum, recordId?: string) {
    ...
    CorePostService.Get(recordId ?? currentRecordId)
      .then((res) => {
        if (res.isSuccess) {
          loadData(
  ...
  function add(formDataParam: CorePostEntity) {
    ...
    CorePostService.Add(formDataParam)
      .then((res) => {
        if (res.isSuccess) {
  ...
  function edit(formDataParam: CorePostEntity) {
    ...
    CorePostService.Update(formDataParam)
      .then((res) => {
        if (res.isSuccess) {
          useToastState.showToast(NotifyTypeEnum.success, res.message);
          getData();
          ...
          {authorizeAction.requestApproval && (
            <ButtonTcd
              ...
              onClick={() => {
                ...
                CorePostService.RequestApprove({
              ...
              onClick={() => {
                useConfirmModalState.confirm({
                  ...
                  onOk: (note?: string) => {
                    ...
                    CorePostService.CancelRequestApproval({
                      recordId: formData.corePostId,
                      note: note,
                    })
                      .then((res) => {
              ...
              onClick={() => {
                useConfirmModalState.confirm({
                  ...
                  onOk: () => {
                    ...
                    CorePostService.Delete(formData.corePostId)
                      .then((res) => {
                        if (res.isSuccess) {
                          useToastState.showToast(
                            NotifyTypeEnum.success,
          ...
          <UpLoadTcd
            ...
            onChange={(dataPram: InputChangeModel) => {
              dataPram.value.length > 0 &&
                CorePostService.UploadImage(dataPram.value[0]).then((res) => {
                  if (res.isSuccess) {
                    useToastState.showToast(
                      NotifyTypeEnum.success,
